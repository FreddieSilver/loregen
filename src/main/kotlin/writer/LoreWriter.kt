    ...
    fun writeLore(events: List<Event>) {
        ...
        events.sortedBy { it.year }.forEach { event ->
            val text = when (event) {
                ...
                is Event.Marriage -> "In the year ${event.year}, ${event.partner1.name.toText()} (${event.partner1.age} Years Old) and ${event.partner2.name.toText()} (${event.partner2.age} Years Old) were united in marriage. Open da champagne niggas"
            }
            println(text)
        }
    }
